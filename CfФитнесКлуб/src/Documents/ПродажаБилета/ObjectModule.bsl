...
Процедура ОбработкаПроведения(Отказ,Режим)
	...
	Запрос.Текст =
		...
		|		КоличествоПосещений,
		|	ПродажаБилетаПозицииПродажи.Номенклатура.ВидУпражнения КАК ВидУпражнения
		|ИЗ
	...
	//СвойстваНоменклатуры = СвойстваНоменклатуры(УдалитьНоменклатура);
	...
	Движение.ВидУпражнения = ВыборкаДетальныеЗаписи.ВидУпражнения;
	Движение.КоличествоПосещений = ВыборкаДетальныеЗаписи.КоличествоПосещений;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если БаллыКСписанию <> 0 Тогда
		Если БаллыКСписанию > СуммаПродажи Тогда
			...
			Сообщение.Текст = "Баллы не должны превышать сумму продажи по билету!";
			Сообщение.УстановитьДанные(ЭтотОбъект);
			Сообщение.Поле = "БаллыКСписанию";
			Сообщение.Сообщить();
		КонецЕсли;
		Если СуммаПродажи <> 0 Тогда
			ДоляБаллов = БаллыКСписанию/СуммаПродажи * 100;
		...
		Если НЕ ЗначениеЗаполнено(Клиент) Тогда
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтрШаблон("Необходимо указать клиента для списания баллов");
				Сообщение.УстановитьДанные(ЭтотОбъект);
